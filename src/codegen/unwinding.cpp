...
Box* fastLocalsToBoxedDict() {
    for (PythonFrameIterator& frame_info : unwindPythonFrames()) {
        ...
        if (frame_info.getId().type == PythonFrameId::COMPILED) {
            ...
            // Get the boxedLocals object for the frame, or create it if it doesn't already exist.
            for (const LocationMap::LocationTable::LocationEntry& e : cf->location_map->names[BOXED_LOCALS_NAME].locations) {
                if (e.offset < offset && offset <= e.offset + e.length) {
                    break;
                }
            }
            ...
            boxedLocals = boxedLocalsForInterpretedFrame((void*)fame_info.getId().bp);
        } else {
            abort();
        }
        ...
        for (const auto& p : d) {
            Box* varname = p.first;
            Box* value = p.second;
            setitem(boxedLocals, varname, value);
        }
